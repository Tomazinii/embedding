...
def embedding_chunks(embeddings_model, chunks,model_name, batch_size: int = 1):
...
#         futures = [
#             executor.submit(process_query, data, embedding_models, method_names, table_name)
...
#                 results.append(output)
#                 best_results.append(best_config)
#                 print(f"✔ Finalizado query {idx + 1}/{len(query_dataset)}")
#             except Exception as e:
#                 print(f"❌ Erro na query {idx + 1}: {e}")

#     # Salvando resultados
#     with open("best_results.json", "w") as f:
#         json.dump({"best_results": best_results}, f, ensure_ascii=False, indent=4)
...
def main():
    ...
    # for idx, method in enumerate(chunks_methods):
    #     print(f"{method["chunk_method"]} chunk method started ------> {idx + 1}/{len(chunks_methods)}")
    #     print(f" chunk method started")
    #     chunks = method["method"].execute()
    #     print("comprimento dos chunks", len(chunks))
        
    #     for model in embedding_models:
    #         print(f"{model.get("model_name")} embedding model started ------> {idx + 1}/{len(embedding_models)}")
    #         chunks_embeded = embedding_chunks(embeddings_model=model.get("model"), chunks=chunks, model_name=model.get("model_name"))
            
            
    #     insertion_vector_store(data_embed=chunks_embeded,model_names=model_names, vector_table_name=iteraction_id)
    #     print(f"dados usando metodo {method["chunk_method"]} inseridos na tabela {iteraction_id}")
